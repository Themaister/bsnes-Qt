...
CheatEditorWindow::CheatEditorWindow() {
  ...
  cheatEnableBox->setEnabled(false);
  cheatEnableBox->setToolTip("Unchecking this disables all cheat codes");
  cheatEnableBox->setChecked(true);  //SNES::cheat.enabled());
...
void CheatEditorWindow::synchronize() {
  ...
  findButton->setEnabled(application.cartridgeLoaded);
  if(application.cartridgeLoaded == false && cheatImportWindow->isVisible()) cheatImportWindow->close();
}
