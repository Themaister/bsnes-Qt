...
void bsnes_video_refresh(const uint16_t *data, unsigned width, unsigned height) { interface.video_refresh(data, width, height); }
void bsnes_audio_sample(uint16_t left, uint16_t right) { interface.audio_sample(left, right); }
void bsnes_input_poll() { interface.input_poll(); }
int16_t bsnes_input_state(bool port, unsigned device, unsigned index, unsigned id) { interface.input_poll(port, device, index, id); }
