...
int16_t Gamepad::status(unsigned index, unsigned id) const {
  if(config().input.allowInvalidInput == false) {
    ...
    if(id == SNES_DEVICE_ID_JOYPAD_DOWN && up.cachedState) return 0;
    if(id == SNES_DEVICE_ID_JOYPAD_RIGHT && left.cachedState) return 0;
  }

  switch(id) {
    case SNES_DEVICE_ID_JOYPAD_UP: return up.cachedState;
    case SNES_DEVICE_ID_JOYPAD_DOWN: return down.cachedState;
    case SNES_DEVICE_ID_JOYPAD_LEFT: return left.cachedState;
    case SNES_DEVICE_ID_JOYPAD_RIGHT: return right.cachedState;
    case SNES_DEVICE_ID_JOYPAD_A: return a.cachedState | turboA.cachedState;
    case SNES_DEVICE_ID_JOYPAD_B: return b.cachedState | turboB.cachedState;
    case SNES_DEVICE_ID_JOYPAD_X: return x.cachedState | turboX.cachedState;
    case SNES_DEVICE_ID_JOYPAD_Y: return y.cachedState | turboY.cachedState;
    case SNES_DEVICE_ID_JOYPAD_L: return l.cachedState | turboL.cachedState;
    case SNES_DEVICE_ID_JOYPAD_R: return r.cachedState | turboR.cachedState;
    case SNES_DEVICE_ID_JOYPAD_SELECT: return select.cachedState;
    case SNES_DEVICE_ID_JOYPAD_START: return start.cachedState;
  }
  return 0;
}
...
int16_t Asciipad::status(unsigned index, unsigned id) const {
  ...
  switch(id) {
    ...
    case SNES_DEVICE_ID_JOYPAD_LEFT: return left.cachedState;
    case SNES_DEVICE_ID_JOYPAD_RIGHT: return right.cachedState;
    case SNES_DEVICE_ID_JOYPAD_A: return a.cachedState;
    case SNES_DEVICE_ID_JOYPAD_B: return b.cachedState;
    case SNES_DEVICE_ID_JOYPAD_X: return x.cachedState;
    case SNES_DEVICE_ID_JOYPAD_Y: return y.cachedState;
    case SNES_DEVICE_ID_JOYPAD_L: return l.cachedState;
    case SNES_DEVICE_ID_JOYPAD_R: return r.cachedState;
    case SNES_DEVICE_ID_JOYPAD_SELECT: return select.cachedState;
    case SNES_DEVICE_ID_JOYPAD_START: return start.cachedState | slowMotion.cachedState;
  }
  return 0;
}
...
int16_t Mouse::status(unsigned index, unsigned id) const {
  switch(id) {
    case SNES_DEVICE_ID_MOUSE_X: return x.cachedState;
    case SNES_DEVICE_ID_MOUSE_Y: return y.cachedState;
    case SNES_DEVICE_ID_MOUSE_LEFT: return left.cachedState;
    case SNES_DEVICE_ID_MOUSE_RIGHT: return right.cachedState;
  }
  return 0;
}
...
int16_t SuperScope::status(unsigned index, unsigned id) const {
  switch(id) {
    case SNES_DEVICE_ID_SUPER_SCOPE_X: return x.cachedState;
    case SNES_DEVICE_ID_SUPER_SCOPE_Y: return y.cachedState;
    case SNES_DEVICE_ID_SUPER_SCOPE_TRIGGER: return trigger.cachedState;
    case SNES_DEVICE_ID_SUPER_SCOPE_CURSOR: return cursor.cachedState;
    case SNES_DEVICE_ID_SUPER_SCOPE_TURBO: return turbo.cachedState;
    case SNES_DEVICE_ID_SUPER_SCOPE_PAUSE: return pause.cachedState;
  }
  return 0;
}
...
int16_t Justifier::status(unsigned index, unsigned id) const {
  switch(id) {
    case SNES_DEVICE_ID_JUSTIFIER_X: return x.cachedState;
    case SNES_DEVICE_ID_JUSTIFIER_Y: return y.cachedState;
    case SNES_DEVICE_ID_JUSTIFIER_TRIGGER: return trigger.cachedState;
    case SNES_DEVICE_ID_JUSTIFIER_START: return start.cachedState;
  }
  return 0;
}
