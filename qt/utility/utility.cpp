...
void Utility::updateSystemState() {
  ...
  if(application.cartridgeLoaded == false) {
...
void Utility::acquireMouse() {
  if(application.cartridgeLoaded) {
    if(config().snes.controller_port1 == SNES_DEVICE_MOUSE
...
void Utility::updateControllers() {
  snes_set_controller_port_device(0, config().snes.controller_port1);
  snes_set_controller_port_device(1, config().snes.controller_port2);
