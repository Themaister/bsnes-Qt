...
MainWindow::MainWindow() {
  ...
  setWindowTitle(string() << ProgramName);
...
void MainWindow::syncUi() {
  system_power->setEnabled(application.cartridgeLoaded);
  system_power->setChecked(application.power == true);
  system_power->setEnabled(application.cartridgeLoaded);
  system_reset->setEnabled(application.cartridgeLoaded && application.power);
  ...
  tools_movies->setEnabled(application.cartridgeLoaded && cartridge.saveStatesSupported());
...
void MainWindow::setPort1Multitap() {
  ...
  config().snes.controller_port1 = SNES_DEVICE_MULTITAP;
  utility.updateControllers();
}

void MainWindow::setPort1Mouse() {
  config().input.port1 = ControllerPort1::Mouse;
  config().snes.controller_port1 = SNES_DEVICE_MOUSE;
  utility.updateControllers();
}

void MainWindow::setPort2None() {
  config().input.port2 = SNES_DEVICE_NONE;
  config().snes.controller_port2 = SNES_DEVICE_NONE;
  utility.updateControllers();
}

void MainWindow::setPort2Gamepad() {
  config().input.port2 = ControllerPort2::Gamepad;
  config().snes.controller_port2 = SNES_DEVICE_JOYPAD;
  utility.updateControllers();
}
